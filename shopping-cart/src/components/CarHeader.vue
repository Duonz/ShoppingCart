        ...
        <div class="navbar">
            ...
            <div class="navbar-right-container" style="display: flex;">
                <div class="navbar-menu-container">
                    ...
                    <span class="navbar-link" v-text="returnName" 
                        v-if="returnName"></span>
...
export default {
    data(){
        return{
            userName:'',
            userPwd:'',
            errorTip:false, //提示：用户名或者密码错误
            loginModelFlag:false, //登陆框 默认不显示
        }
    },
    mounted(){
        this.checkLogin();
    ...
    computed:{
        returnName(){
            return this.$store.state.returnName;
        },
        cartCount(){
    ...
    methods:{
        //校验是否登陆，不登陆就不能加入购物车，登陆后才能加入购物车
        checkLogin(){
            axios.get('/users/checkLogin').then((response)=>{
                let res = response.data;
                if(res.status == '0'){
                    this.$store.commit('updateUseriInfor',res.result);
                    this.getcartCount(); //登陆成功后,查询购物车的数量
                }
            });
        },
        //登陆
        login(){
            ...
            axios.post('/users/login',{
                ...
                let res = response.data;
                if(res.status == '0'){ // 成功 
                    this.errorTip = false; //不提示
                    this.loginModelFlag = false;
                    //把登陆成功后拿到的用户名保存起来
                     this.$store.commit('updateUseriInfor',res.result.userName);
                    this.getcartCount(); //登陆成功后,查询购物车的数量
                }else{
                    this.errorTip = true;
                }
        ...
        logOut(){
            axios.post('/users/logout').then((response)=>{
                let res = response.data;
                if(res.status == '0'){
                     this.$store.commit('updateUseriInfor','')
                }
            })
        },
        //查看购物车的数量
