<template>
    <div>
            ...
            <div class="container">
                ...
                <!-- process step -->
                    ...
                    <div class="addr-list-wrap">
                        ...
                        <ul>
                            <li v-for="(item,index) in addressListFilter" :key="index"
                                ...
                                @click="checkIndex=index;seladdressId=item.addressId"
                            >
                                <dl>
                                    <dt>{{item.userName}}</dt>
                    ...
                    <div class="lemall-msg-info hidden">
                        <span>您选择的区域不在我们的交付区域内。请在我们的配送区域内选择另一个发货地址。</span>
                    </div>
                    ...
                    <div class="next-btn-wrap">
                        <router-link class="btn btn--m btn--red" :to="{path:'orderConfirm',query:{'addressId':seladdressId}}">下一步</router-link>
                    </div>
                </div>
            </div>
        </div>
        ...
        <modal :mdShow=deleisMdShow @close="deleisMdShow=false">
            <p slot="message">
                地址列表至少要有一条数据，已无法继续删除
            </p>
            <div slot="btnGroups">
                <a class="btn btn--m" href="javascript:;" @click="deleisMdShow = false">OK</a>
            </div>
        </modal>
    ...
    methods:{
        ...
        delAddress(){            
            axios.post('/users/delAddress',{
                ...
                let res = response.data;
                if(res.status == '0'){
                    console.log('deladdress success');
                    this.isMdShow = false;
                    this.init()
                }
            })
